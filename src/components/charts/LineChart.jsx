...
export default function LineChart({
  points = [], target, xLabel, color = 'rgb(37 99 235)', targetLabel = 'Meta',
}) {
  if (points.length < 2) {
    return <p className="text-xs text-slate-400">Datos insuficientes para proyectar.</p>;
  }
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img">
        ...
        {ticks.map((t) => (
          <g key={t}>
            <line
              ...
              stroke="rgb(241 245 249)" strokeWidth="1"
            />
            <text
              x={PAD_L - 4} y={sy(t) + 3}
              textAnchor="end" fontSize="7" fill="rgb(148 163 184)"
            >
              {fmtCompact(t)}
            </text>
          </g>
        ...
        {target > 0 && target <= maxY && (
          <g>
            <line
              ...
              stroke="rgb(220 38 38)" strokeWidth="1" strokeDasharray="3 2"
            />
            <text
              x={W - PAD_R} y={sy(target) - 3}
              textAnchor="end" fontSize="7" fill="rgb(220 38 38)" fontWeight="600"
            >
              {targetLabel}
            </text>
          </g>
        ...
        <text x={PAD_L} y={H - 6} fontSize="7" fill="rgb(148 163 184)">{minX}</text>
        <text x={W - PAD_R} y={H - 6} textAnchor="end" fontSize="7" fill="rgb(148 163 184)">
          {maxX}
        </text>
      </svg>
      {xLabel && (
        <p className="mt-1 text-center text-[10px] text-slate-400">{xLabel}</p>
      )}
    </div>
  );
}
